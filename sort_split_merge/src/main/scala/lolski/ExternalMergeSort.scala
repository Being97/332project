...
object ExternalMergeSort {
  ...
  def split_sort_split(input: String, tmpDir: String, output: String, linesPerChunk: Int)
...
// https://groups.google.com/g/scala-user/c/zp52kkbwD9g
  def sortChunk(path: String): String = {
...
*/
/*
  def Byte(elemA: String, elemB: String): Boolean = {
    val aByte = elemA.take(10).map(_.toByte).toList
    val bByte = elemB.take(10).map(_.toByte).toList
